...
type PackageResponse = { package: ContentPackage; history?: ContentPackage[]; blogUrl?: string; imagePreview?: string };

const initialAnswers: DailyAnswers = { objective: "auto", treatment: "auto", special: "" };
...
export default function ContentAdminPage() {
  ...
  function resetDraft() {
    ...
    setCurrent(null); setSelectedTopic(null); setTopics([]); setFormats([]); setDirty(false); setEditing(false); setNotice(null); setTopicNotice(""); setImagePreview(""); setImageUnavailable(false); setRightsConfirmed(false); setBlogUrl(""); setHistoryOpen(false);
  }

  function loadHistory(item: ContentPackage) {
    if (running.current || !confirmDiscard()) return;
    setCurrent(structuredClone(item)); setSelectedTopic(item.topic); setActiveFormat(availableFormats(item)[0]?.id || "blog"); setDirty(false); setEditing(false); setNotice(null); setImagePreview(""); setImageUnavailable(false); setRightsConfirmed(false); setBlogUrl(item.blogSlug ? `/admin/blog?draft=${encodeURIComponent(item.blogSlug)}` : ""); setHistoryOpen(false);
  ...
  function edit(update: (item: ContentPackage) => ContentPackage) {
    ...
    setDirty(true); setNotice(null); setBlogUrl("");
  }

  function applyPackage(item: ContentPackage, options?: { saved?: boolean; preview?: string }) {
    setCurrent(item); setDirty(!options?.saved); setEditing(false);
