...
export type SeoCheck = { label: string; passed: boolean; points: number; guidance: string };
...
function technicalSeoChecks(post: Partial<BlogPost>): SeoCheck[] {
  ...
  return [
    ...
    {label:"SEO · Focus topic",passed:Boolean(topic),points:10,guidance:"Set one natural patient search topic rather than many keyword variants."},
...
function readabilityChecks(post: Partial<BlogPost>): SeoCheck[] {
  ...
  return [
    ...
    {label:"Readability · Sentence length",passed:avg>0&&avg<=24,points:22,guidance:`Average sentence length: ${avg?avg.toFixed(1):"0"} words.`},
    {label:"Readability · Dense sentences",passed:longRatio<=.25,points:18,guidance:`${Math.round(longRatio*100)}% of sentences are over 28 words. Split dense sentences where useful.`},
    {label:"Readability · Short paragraphs",passed:longest>0&&longest<=120,points:18,guidance:`Longest paragraph: ${longest} words. Keep mobile reading comfortable.`},
...
function trustChecks(post: Partial<BlogPost>): SeoCheck[] {
  ...
  return [
    ...
    {label:"Trust · No absolute promises",passed:!hasAbsolute,points:20,guidance:hasAbsolute?"Remove absolute promotional or outcome promises.":"No obvious absolute outcome language detected."},
    {label:"Trust · No certainty diagnosis",passed:!hasCertainty,points:20,guidance:hasCertainty?"Avoid telling a reader they definitely have a condition without examination.":"No obvious certainty-based diagnosis language detected."},
    {label:"Trust · Assessment-first wording",passed:assessment,points:15,guidance:"Remind readers that diagnosis and treatment suitability depend on examination or individual findings."},
  ];
}
...
export function seoChecks(post: Partial<BlogPost>): SeoCheck[] {
  ...
  const priorities=[...seo,...readability,...trust].filter((check)=>!check.passed).sort((a,b)=>b.points-a.points).slice(0,7);
  return [
