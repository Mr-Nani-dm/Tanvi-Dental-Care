import { topicImageConcept } from "./contentTopicPresentation";
...
Return exactly ${CONTENT_POLICY.topicCount} distinct useful topic candidates. Prioritise narrow patient questions over generic treatment overviews: preparation for a consultation, a specific care question, or questions about options. Every idea must identify one distinct intent and angle relevant to patients seeking care in Mangalagiri, Andhra Pradesh, India. Use a natural specific primaryKeyword; avoid keyword stuffing, invented neighbourhood facts, fabricated local statistics or unsupported best/number-one claims. Local relevance is clinic location and a useful patient task, not a claim that this topic is popular. Do not force three minor rewrites of the same question. Compare topic intent, treatment, angle and title with the supplied existing blog and content history. A covered subject should reuse/update its existing page or create a genuinely different social angle, not another near-duplicate blog. Recommend a blog only when a useful detailed new explanation is justified; short reminders, clinic updates and already-covered subjects should be social/GBP only. A short idea is not made blog-worthy by adding filler.
...
Use suggestedImageConcept as a starting point and tailor the illustration to the selected topic’s specific question. Describe only the permitted objects you want drawn in imageBrief.description. Do not repeat negative safety lists or forbidden imagery names in the brief; the image API adds the safety constraints separately. Image brief is an original simple non-graphic educational concept only: no people, doctor faces, patient photos, testimonials, before/after, product logos or imitation of copyrighted artwork. For a doctor profile choose kind doctor with naga-swathi or prathap-naidu and request the approved real portrait only. Never ask image generation to invent or replace a doctor face. The image remains for human review.
If correction and previousDraft are supplied, revise exactly those failures while preserving the selected formats, factual accuracy and source restrictions.`, {
    clinicFacts, contentPolicy: CONTENT_POLICY, safetyPolicy: CONTENT_SAFETY_POLICY,
    suggestedImageConcept: topicImageConcept(context.topic), approvedInternalUrls: INTERNAL_URLS, topic: context.topic, selectedFormats: context.formats, sources,
