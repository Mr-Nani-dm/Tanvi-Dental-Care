...
Images are decoded and re-encoded with Sharp to WebP, metadata stripped, maximum 1,200px and 3 MB, then returned as an ephemeral browser-held preview. No generated pixels enter GitHub until the administrator sees the image and explicitly approves public storage. Approval verifies a server HMAC bound to the image SHA-256, exact reviewed draft and branch, expiring after 30 minutes. Approved images are saved under `public/images/content/YYYY/MM/`; the committed public URL becomes available after the branch rebuild. A generated image remains marked for human review. Old asset paths are unchanged.
